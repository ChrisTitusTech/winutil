...
Searching also queries the package-manager catalogs in the background. Expand **Package Manager Results** to see up to 15 additional matches for the selected manager. WinGet searches use its community source; Microsoft Store results are not included. Category filters show curated apps only.

While a catalog search is running, further typing replaces the pending search with your latest text. If a catalog request fails, searching for the same text again retries it.

Switching package managers refreshes the results and deselects apps unavailable from the new manager. Selected search results remain available while you search for other apps, but these session-only entries are excluded from configuration exports. Catalog searches are disabled in offline mode and while a package operation is running.
