...
Source files are bind-mounted into the container, so edits on the host are picked up immediately by the dev server — no rebuild needed for normal content or code changes. Rebuild the image (`docker compose build`) only after changing `package.json`, `package-lock.json`, or the `Dockerfile`.
