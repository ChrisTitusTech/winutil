...
Describe "Get-WinUtilSelectedPackages" {
    ...
    It "uses winget IDs when winget is preferred" {
        ...
        $result = Get-WinUtilSelectedPackages `
            -PackageList $packages `
            -Preference "Winget"

        (@($result["Winget"]) -join "|") | Should -Be "Git.Git|VideoLAN.VLC"
        @($result["Choco"]).Count | Should -Be 0
        @($result["Github"]).Count | Should -Be 0
        @($result["Npm"]).Count | Should -Be 0
    }
    ...
    It "routes GitHub and npm installers into their dedicated buckets" {
        $githubPackage = [pscustomobject]@{
            content = "EmuDeck for Windows"
            installType = "github"
            repo = "EmuDeck/emudeck-electron"
            assetPattern = "EmuDeck-Setup-*.exe"
        }

        $npmPackage = [pscustomobject]@{
            content = "Prismcast"
            installType = "npm"
        ...
        $result = Get-WinUtilSelectedPackages `
            -PackageList @($githubPackage, $npmPackage) `
            -Preference "Winget"

        $githubResults = @($result["Github"])
        $npmResults = @($result["Npm"])

        @($result["Winget"]).Count | Should -Be 0
        @($result["Choco"]).Count | Should -Be 0

        $githubResults.Count | Should -Be 1
        $githubResults[0].content | Should -Be "EmuDeck for Windows"
        $githubResults[0].repo | Should -Be "EmuDeck/emudeck-electron"
        $githubResults[0].assetPattern | Should -Be "EmuDeck-Setup-*.exe"

        $npmResults.Count | Should -Be 1
        $npmResults[0].content | Should -Be "Prismcast"
        $npmResults[0].npmPackage | Should -Be "prismcast"
    }
...
Describe "Get-WinUtilPackageLogSummary" {
    It "summarizes GitHub installer packages using content and release metadata" {
        ...
        $result = Get-WinUtilPackageLogSummary `
            -Packages @($package) `
            -Preference "Winget"

        $result | Should -Be "EmuDeck for Windows (GitHub: EmuDeck/emudeck-electron, asset: EmuDeck-Setup-*.exe)"
    }

    It "summarizes npm installer packages using content and npmPackage" {
        $package = [pscustomobject]@{
            content = "Prismcast"
            installType = "npm"
            npmPackage = "prismcast"
        ...
        $result | Should -Be "Prismcast (npm: prismcast)"
    }

    It "continues to summarize Winget packages" {
        $package = [pscustomobject]@{
            content = "Git"
            winget = "Git.Git"
            choco = "git"
    ...
    It "uses Chocolatey when Chocolatey is preferred and available" {
        $package = [pscustomobject]@{
            content = "Git"
            winget = "Git.Git"
            choco = "git"
    ...
    It "uses Winget when Chocolatey is preferred but unavailable" {
        $package = [pscustomobject]@{
            content = "Firefox"
            winget = "Mozilla.Firefox"
            choco = "na"
        ...
        $result | Should -Be "Firefox (winget: Mozilla.Firefox)"
    }
}
...
Describe "Install-WinUtilProgramChoco" {
    ...
    It "starts choco with uninstall arguments" {
        ...
        Should -Invoke -CommandName Start-Process -Times 1 -Exactly -ParameterFilter {
            $FilePath -eq "choco" -and
            $ArgumentList -eq "uninstall git -y"
        }
    }
}
