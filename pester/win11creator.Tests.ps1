...
Describe "Win11 Creator setup media" {
    ...
    It "adds eligible drivers to one install.wim index and does not create `$WinpeDriver$" {
        ...
        try {
            ...
            ($logs -join '|') | Should -Match 'Exported 9 of 11 driver packages \(2 excluded\)'
    ...
    It "injects SCSIAdapter storage drivers into boot.wim index 2 and skips Extension companions" {
        ...
        try {
            ...
            @($script:dismCalls | Where-Object { $_ -match '/Add-Driver' }).Count | Should -Be 7
            ($logs -join '|') | Should -Match "Excluding extension-class driver package '.*iastorhsa_ext"
            ($logs -join '|') | Should -Match 'Added 2 of 2 driver packages to boot.wim'
            ($logs -join '|') | Should -Match 'Added 5 of 5 driver packages to install.wim'
        } finally {
            Remove-Item Function:\dism.exe -ErrorAction SilentlyContinue
            Remove-Item -Path $contentRoot -Recurse -Force -ErrorAction SilentlyContinue
        }
    ...
    It "excludes Class=Extension driver packages from Add-Driver regardless of vendor or case" {
        ...
        try {
            ...
            ($logs -join '|') | Should -Match 'Exported 1 of 3 driver packages \(2 excluded\)'
